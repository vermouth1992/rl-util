...
class CNN_WGAN_GP(WGAN_GP):
    ...
    def _make_generator(self) -> tf.keras.Model:
        ...
        # 4 x 4 x 64
        for _ in range(2):
            model.add(tf.keras.layers.Conv2DTranspose(filters=64, kernel_size=3, strides=2,
                                                      data_format=self.data_format, padding='same'))
            model.add(tf.keras.layers.BatchNormalization(axis=-1))
            model.add(tf.keras.layers.LeakyReLU())
        model.add(tf.keras.layers.Conv2DTranspose(filters=self.channel, kernel_size=3, strides=2,
    ...
    def _make_discriminator(self) -> tf.keras.Model:
        model = tf.keras.Sequential([
            ...
            tf.keras.layers.Conv2D(filters=64, kernel_size=3, strides=2, data_format=self.data_format, padding='same'),
            tf.keras.layers.LeakyReLU(),
            tf.keras.layers.Conv2D(filters=64, kernel_size=3, strides=2, data_format=self.data_format, padding='same'),
            tf.keras.layers.LeakyReLU(),
            tf.keras.layers.Conv2D(filters=64, kernel_size=3, strides=2, data_format=self.data_format, padding='same'),
...
class ShowImage(tf.keras.callbacks.Callback):
    def __init__(self, logdir, model: CNN_WGAN_GP):
        super(ShowImage, self).__init__()
        self.writer = SummaryWriter(logdir=logdir)
        self.x = model.prior.sample(100)
...
def cnn_cvae(dataset,
    ...
    model = CNN_WGAN_GP(image_size=image_size)
    callback = ShowImage(logdir=f'data/{dataset}_cnn_wgan_gp', model=model)
    model.fit(x=x_train, epochs=epochs, batch_size=batch_size, callbacks=[callback])
