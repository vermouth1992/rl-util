...
class DDPGGPAgent(tf.keras.Model):
    def __init__(self,
                 obs_spec,
                 act_spec,
                 num_q_ensembles=2,
        ...
        super(DDPGGPAgent, self).__init__()
        ...
        self.gp_threshold = tf.Variable(initial_value=gp_threshold, trainable=False)
        self.alpha = rlu.nn.LagrangeLayer(initial_value=1.0)
        self.alpha_optimizer = tf.keras.optimizers.Adam(lr=q_lr)
    ...
    def log_tabular(self):
        ...
        self.logger.log_tabular('AlphaLoss', average_only=True)
    ...
    def _update_q_nets(self, obs, act, next_obs, done, rew):
        ...
        with tf.GradientTape() as q_tape:
            ...
            loss = tf.reduce_mean(q_values_loss)
            gradient_norm = self._compute_gp(obs)
            # compute gradient norm
            gp_loss = tf.reduce_mean(gradient_norm)
            loss = loss + gp_loss * self.alpha()
        ...
        with tf.GradientTape() as tape:
            alpha = self.alpha()
            alpha_loss = -tf.reduce_mean(alpha * (gradient_norm - self.gp_threshold))
        alpha_grad = tape.gradient(alpha_loss, self.alpha.trainable_variables)
        self.alpha_optimizer.apply_gradients(zip(alpha_grad, self.alpha.trainable_variables))
    ...
    def _compute_gp(self, obs):
        pi_action = self.policy_net(obs)
